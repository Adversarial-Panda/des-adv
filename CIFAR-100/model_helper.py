...
def wrap_model_with_transform(model_name, model):
    if any(x in model_name for x in ['inc']): 
...
def load_model_hub(model_name): 
    if model_name in ADAEA_MODELS_DICT: 
        ...
        model = get_models('cifar100', model_name, OUR_MODELS_DICT[model_name])  
        state_dict = torch.load(f"checkpoints/{model_name}_cifar100.pth", map_location="cpu")
